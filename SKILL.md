...
- Complete the `references/lifecycle-safety.md` path and mount ledger. Derive runtime UID/GID from the published OCI configuration or an explicit Compose `user`; keep mutable host paths package-local and confined before creation, permission changes, or cleanup. Do not recursively change ownership on an unconfined or symlinked path.
- Create or validate the exact source file for every file bind before Compose starts. Prove each generated secret format against the application contract, keep stable secrets across upgrade, and URL-encode credentials before embedding them in connection URLs.
- Replace placeholders with real product metadata and meaningful translations in all required locales. English fields must contain English, and logo/license handling must identify the upstream asset and redistribution basis or explicitly retain the neutral placeholder.
