...
- Replace placeholders with real product metadata and meaningful translations in all required locales. English fields must contain English. Record the application license; when it has material use restrictions, name and link it in the README instead of keeping it only in machine-readable evidence. When an asset's redistribution basis is verified, record it; for an unresolved asset license, use the neutral placeholder immediately rather than shipping the asset with a future-confirmation note.
