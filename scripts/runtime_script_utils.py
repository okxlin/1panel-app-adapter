...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--only-needed", action="store_true", help="Skip init.sh when there is no directory or ownership setup")
    args = parser.parse_args(argv[1:])
    try:
        if args.replace_init and not args.finalize_lifecycle:
            parser.error("--replace-init requires --finalize-lifecycle")
        if args.only_needed and not args.dir_owner and not args.fixed_dir_owner and not collect_runtime_path_fields(load_version_data(Path(args.version_data))):
            return 0
